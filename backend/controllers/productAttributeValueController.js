...
export const createProductAttributeValue = async (req, res) => {
  try {
    ...
    if (!product_id || !attribute_id || !attribute_value_id)
      return res.status(400).json({ message: "product_id, attribute_id, attribute_value_id are all required" });
    ...
    const created = await prisma.productAttributeValue.create({
      data: { productId: product_id, attributeId: attribute_id, attributeValueId: attribute_value_id, storeId: store_id || null },
      include: {
        attribute: { select: { id: true, name: true } },
        attributeValue: { select: { id: true, value: true } },
      },
    });
    logAudit({ tenantId, userId: req.user.id, module: "product_attribute_values", action: "create", recordId: created.id, newValues: { product_id, attribute_id, attribute_value_id }, req });
