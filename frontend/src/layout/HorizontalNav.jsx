...
export default function HorizontalNav() {
  ...
  const DropItem = ({ to, label }) => (
    <Link
      to={to}
      className={`hn-drop-item ${location.pathname === to ? "active" : ""}`}
  ...
  const MpItem = ({ to, label }) => (
    <Link
      to={to}
      className={`hn-mp-item ${location.pathname === to ? "active" : ""}`}
    ...
    <div id="hnavWrap" ref={navRef} style={{ position: "relative", flexShrink: 0, zIndex: 20 }}>
      <div className="hnav">
        ...
        <div className="hn-items-wrap">
          ...
          {/* SALES dropdown */}
          <div
            className={`hn-item has-drop ${openDrop === "sales" ? "open" : ""} ${isActive("/sales", "/expenses") ? "active" : ""}`}
            onClick={() => toggleDrop("sales")}
          >
            <span className="hn-label">Sales</span>
            <svg className="hn-arr" viewBox="0 0 24 24"><polyline points="6 9 12 15 18 9" /></svg>
          ...
          {/* PURCHASES dropdown */}
          <div
            className={`hn-item has-drop ${openDrop === "purchases" ? "open" : ""} ${isActive("/purchases") ? "active" : ""}`}
            onClick={() => toggleDrop("purchases")}
          >
          ...
          {/* INVENTORY dropdown */}
          <div
            className={`hn-item has-drop ${openDrop === "inventory" ? "open" : ""} ${isActive("/products", "/inventory") ? "active" : ""}`}
            onClick={() => toggleDrop("inventory")}
          >
          ...
          <div
            className={`hn-item has-drop ${openDrop === "master" ? "open" : ""} ${isActive("/customers", "/suppliers", "/categories", "/brands", "/units", "/taxes", "/expense-categories") ? "active" : ""}`}
            onClick={() => toggleDrop("master")}
          >
          ...
          {/* RESTAURANT dropdown */}
          <div
            className={`hn-item has-drop ${openDrop === "restaurant" ? "open" : ""} ${isActive("/restaurant") ? "active" : ""}`}
            onClick={() => toggleDrop("restaurant")}
          >
          ...
          {(hasPermission("stores.view") || hasPermission("roles.view") || hasPermission("users.view")) && (
            <div
              className={`hn-item has-drop ${openDrop === "admin" ? "open" : ""} ${isActive("/stores", "/roles", "/users") ? "active" : ""}`}
        ...
        <button
          ...
          style={{ background: mobileOpen ? "var(--abg)" : undefined, borderColor: mobileOpen ? "var(--accent)" : undefined, color: mobileOpen ? "var(--accent)" : undefined }}
        >
