...
const EMPTY = { store_subscription_plan_id: "", start_date: "", end_date: "", amount: "", payment_status: "paid", status: "active" };
...
export default function StoreSubscriptions() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const res = await api.get("/store-subscriptions/all");
  ...
  useEffect(() => {
    api.get("/store-subscription-plans").then((r) => setPlans(r.data?.data || r.data || [])).catch(() => {});
  }, []);
  ...
  const filtered = useMemo(() => {
    ...
    return data.filter((r) =>
      ...
      (r.tenant?.businessName || "").toLowerCase().includes(s) ||
  ...
  const openEdit = (r) => {
    setForm({
      store_subscription_plan_id: r.store_subscription_plan_id || "",
  ...
  const handleSave = async () => {
    ...
    try {
      const payload = {
        store_subscription_plan_id: form.store_subscription_plan_id || null,
      ...
      if (modal === "edit") {
        await api.put(`/store-subscriptions/${editId}`, payload);
  ...
  const handleDelete = async () => {
    try {
      await api.delete(`/store-subscriptions/${deleteDialog.id}`);
    ...
    <MainLayout>
      <div className="card">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between mb-3">
          <strong className="ct" style={{ fontSize: 15, flexShrink: 0 }}>Store Subscriptions</strong>
          <div className="srch w-full sm:w-auto sm:flex-1 sm:max-w-xs">
            <span className="srch-ic"><Search size={13} /></span>
            <input type="text" placeholder="Search by plan or company…" value={search} onChange={(e) => setSearch(e.target.value)} />
          </div>
          ...
          <table className="w-full">
            <thead>
              <tr>
                <th>Company</th>
                <th>Plan</th>
                <th className="hidden md:table-cell">Max Stores</th>
            ...
            <tbody>
              {loading ? (
                ...
                <tr><td colSpan={8} style={{ textAlign: "center", color: "var(--tx3)", padding: "24px 0" }}>No store subscriptions found</td></tr>
              ) : paged.map((r) => {
                ...
                return (
                  <tr key={r.id} style={{ cursor: "pointer" }} onClick={() => openEdit(r)}>
                    <td style={{ fontSize: 12 }}>{r.tenant?.businessName || "—"}</td>
                    <td><strong style={{ color: "var(--tx)" }}>{r.plan?.name || "—"}</strong></td>
                    <td className="hidden md:table-cell" style={{ textAlign: "center", fontWeight: 700, color: "var(--accent)" }}>{r.plan?.max_stores ?? "—"}</td>
                    ...
                    <td onClick={(e) => e.stopPropagation()} style={{ textAlign: "right" }}>
                      <div style={{ display: "flex", alignItems: "center", justifyContent: "flex-end", gap: 4 }}>
                        ...
                        <button className="hbtn" onClick={(e) => { e.stopPropagation(); setDeleteDialog({ open: true, id: r.id, label: `${r.plan?.name || "subscription"} for ${r.tenant?.businessName || "this company"}` }); }} title="Delete"><Trash2 size={12} /></button>
                      </div>
                    </td>
                  </tr>
                );
      ...
      {modal && (
        <Modal
          ...
          title={modal === "edit" ? "Edit Store Subscription" : "Add Store Subscription"}
          width={500}
          footer={
            <>
        ...
        >
          <div className="fg">
            <label>Store Subscription Plan</label>
            <select name="store_subscription_plan_id" value={form.store_subscription_plan_id} onChange={handleChange}>
              <option value="">— No Plan —</option>
              {plans.map((p) => <option key={p.id} value={p.id}>{p.name} — {p.max_stores} store{p.max_stores !== 1 ? "s" : ""} ({p.code})</option>)}
            </select>
          </div>
          <div className="fg">
