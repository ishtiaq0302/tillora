...
export default function Purchases() {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      toast.error(t("failed_to_load", "purchases"));
    } finally {
      setLoading(false);
    }
  }, []);
    ...
    <MainLayout>
      ...
      {viewPurchase && (
        <Modal
          ...
          title={`Purchase — ${viewPurchase.invoice_no}`}
          columns={{ sm: 1, md: 2 }}
          width={640}
          footer={
            <div style={{ display: "flex", gap: 8 }}>
              ...
              <Button variant="ghost" size="sm" onClick={() => setViewPurchase(null)}>
                {t("close", "common")}
              </Button>
            </div>
          }
        >
          <div className="fg">
            ...
            <p style={{ fontSize: 13, color: "var(--tx)", fontWeight: 600 }}>{viewPurchase.invoice_no}</p>
          </div>
          <div className="fg">
            <label>{t("supplier", "common")}</label>
            <p style={{ fontSize: 13, color: "var(--tx)" }}>{viewPurchase.supplier?.name || "—"}</p>
          </div>
          <div className="fg">
            <label>{t("subtotal", "common")}</label>
            <p style={{ fontSize: 13, color: "var(--tx)" }}>{Number(viewPurchase.subtotal).toLocaleString()}</p>
          </div>
          <div className="fg">
            <label>{t("discount", "common")}</label>
            <p style={{ fontSize: 13, color: "var(--tx)" }}>{Number(viewPurchase.discount).toLocaleString()}</p>
          </div>
          <div className="fg">
            <label>{t("tax", "common")}</label>
            <p style={{ fontSize: 13, color: "var(--tx)" }}>{Number(viewPurchase.tax).toLocaleString()}</p>
          </div>
          <div className="fg">
            <label>{t("shipping", "common")}</label>
            <p style={{ fontSize: 13, color: "var(--tx)" }}>{Number(viewPurchase.shipping || 0).toLocaleString()}</p>
          </div>
          <div className="fg">
            <label>{t("grand_total", "common")}</label>
            <p style={{ fontSize: 15, color: "var(--green)", fontWeight: 700 }}>{Number(viewPurchase.grand_total).toLocaleString()}</p>
          </div>
          <div className="fg">
            <label>{t("payment_status", "common")}</label>
            <span style={{ ...statusStyle(viewPurchase.payment_status), borderRadius: "var(--r)", padding: "3px 10px", fontSize: 12, fontWeight: 600, textTransform: "capitalize", display: "inline-block" }}>{viewPurchase.payment_status}</span>
          </div>
          <div className="fg">
            <label>{t("purchase_status", "purchases")}</label>
            <p style={{ fontSize: 13, color: "var(--tx)", textTransform: "capitalize" }}>{viewPurchase.purchase_status}</p>
          </div>
          <div className="fg">
            <label>{t("date", "common")}</label>
            <p style={{ fontSize: 13, color: "var(--tx)" }}>{viewPurchase.created_at ? new Date(viewPurchase.created_at).toLocaleString() : "—"}</p>
          </div>
          {viewPurchase.notes && (
            <div className="fg" style={{ gridColumn: "1 / -1" }}>
              <label>{t("notes", "common")}</label>
              <p style={{ fontSize: 13, color: "var(--tx)" }}>{viewPurchase.notes}</p>
            </div>
          )}
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between mb-3">
          ...
          <div className="flex items-center justify-between sm:justify-end gap-2 flex-wrap">
            ...
            <Link to="/purchases/create" className="btn btn-p" style={{ fontSize: 12 }}>
              ...
              <span>{t("new_purchase", "purchases")}</span>
            </Link>
          </div>
        </div>
          ...
          <table className="w-full">
            ...
            <tbody>
              {loading ? (
                ...
                <tr>
                  <td colSpan={8} style={{ textAlign: "center", color: "var(--tx3)", padding: "24px 0" }}>
                    {t("no_purchases_found", "purchases")}
                  </td>
                </tr>
              ) : (
                paged.map((r) => {
                  ...
                  return (
                    <tr key={r.id}>
                      ...
                      <td>
                        <span style={{ background: s.bg, color: s.color, borderRadius: "var(--r)", padding: "2px 8px", fontSize: 11, fontWeight: 600, textTransform: "capitalize" }}>{r.payment_status}</span>
                      </td>
                      ...
                      <td>
                        <div style={{ display: "flex", alignItems: "center", gap: 4 }}>
                          <button className="hbtn" onClick={() => setViewPurchase(r)} title="View">
                            <Eye size={13} />
                          </button>
        ...
        {!loading && filtered.length > 0 && (
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between mt-4" style={{ fontSize: 12, color: "var(--tx3)" }}>
            <span>
              ...
              {t("of", "common")} <strong style={{ color: "var(--tx)" }}>{filtered.length}</strong> {t("title", "purchases").toLowerCase()}
            </span>
            {totalPages > 1 && (
