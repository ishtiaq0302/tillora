...
export default function ProductVariants() {
  ...
  const handleDelete = async () => {
    try { await masterService.delete("variants", deleteDialog.id); toast.success("Variant deleted"); load(); }
    ...
    <MainLayout>
      <div className="card">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between mb-3">
          <strong className="ct" style={{ fontSize: 15, flexShrink: 0 }}>Variants</strong>
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            <Button variant="primary" size="sm" onClick={openCreate}><Plus size={13} strokeWidth={2.5} /><span>Add Variant</span></Button>
          </div>
        </div>
          ...
          <table className="w-full">
            ...
            <tbody>
              {loading ? (
                ...
                <tr><td colSpan={5} style={{ textAlign: "center", color: "var(--tx3)", padding: "24px 0" }}>No variants found</td></tr>
              ) : paged.map(r => (
                <tr key={r.id} style={{ cursor: "pointer" }} onClick={() => openEdit(r)}>
                  <td><strong style={{ color: "var(--tx)" }}>{r.name}</strong></td>
                  <td>
                    {r.multi_select ? (
                      <span style={{ background: "var(--abg)", color: "var(--accent)", borderRadius: "var(--r)", padding: "2px 8px", fontSize: 11, fontWeight: 600 }}>Multi</span>
                    ) : (
                  ...
                  <td onClick={e => e.stopPropagation()} style={{ textAlign: "right" }}>
                    <div style={{ display: "flex", alignItems: "center", justifyContent: "flex-end", gap: 4 }}>
                      ...
                      <button className="hbtn" onClick={e => { e.stopPropagation(); setDeleteDialog({ open: true, id: r.id, label: r.name }); }} title="Delete"><Trash2 size={12} /></button>
                    </div>
                  </td>
                </tr>
              ))}
      ...
      {modal && (
        <Modal isOpen onClose={closeModal} title={modal === "edit" ? "Edit Variant" : "Add Variant"} width={480}
