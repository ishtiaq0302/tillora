...
const EMPTY = { product_id: "", batch_no: "", manufacture_date: "", expiry_date: "", quantity: 0, store_id: "" };
...
export default function ProductBatches() {
  ...
  const load = useCallback(async () => {
    ...
    } catch { toast.error("Failed to load product batches"); }
  ...
  const openEdit = (r) => {
    setForm({ product_id: r.product_id, batch_no: r.batch_no, manufacture_date: toInputDate(r.manufacture_date), expiry_date: toInputDate(r.expiry_date), quantity: r.quantity, store_id: r.store_id || "" });
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      const payload = { product_id: form.product_id, batch_no: form.batch_no.trim(), manufacture_date: form.manufacture_date || null, expiry_date: form.expiry_date || null, quantity: Number(form.quantity || 0), store_id: form.store_id || null };
      if (modal === "edit") {
  ...
  const handleDelete = async () => {
    try { await masterService.delete("product-batches", deleteDialog.id); toast.success("Batch deleted"); load(); }
    ...
    <MainLayout>
      <div className="card">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between mb-3">
          <strong className="ct" style={{ fontSize: 15, flexShrink: 0 }}>Product Batches</strong>
          ...
          <table className="w-full">
            ...
            <tbody>
              {loading ? (
                ...
                <tr><td colSpan={7} style={{ textAlign: "center", color: "var(--tx3)", padding: "24px 0" }}>No product batches found</td></tr>
              ) : paged.map(r => {
                ...
                return (
                  <tr key={r.id} style={{ cursor: "pointer" }} onClick={() => openEdit(r)}>
                    ...
                    <td><strong style={{ color: "var(--tx)", fontFamily: "monospace", fontSize: 12 }}>{r.batch_no}</strong></td>
                    <td className="hidden md:table-cell" style={{ color: "var(--tx3)", fontSize: 12 }}>{toDateStr(r.manufacture_date)}</td>
                    <td>
                    ...
                    <td onClick={e => e.stopPropagation()} style={{ textAlign: "right" }}>
                      <div style={{ display: "flex", alignItems: "center", justifyContent: "flex-end", gap: 4 }}>
                        ...
                        <button className="hbtn" onClick={e => { e.stopPropagation(); setDeleteDialog({ open: true, id: r.id, label: r.batch_no }); }} title="Delete"><Trash2 size={12} /></button>
                      </div>
                    </td>
                  </tr>
                );
      ...
      {modal && (
        <Modal isOpen onClose={closeModal} title={modal === "edit" ? "Edit Batch" : "Add Batch"} width={520}
        ...
        >
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
            <div className="fg"><label>Manufacture Date</label><input name="manufacture_date" type="date" value={form.manufacture_date} onChange={handleChange} /></div>
            <div className="fg"><label>Expiry Date</label><input name="expiry_date" type="date" value={form.expiry_date} onChange={handleChange} /></div>
          </div>
          <div className="fg"><label>Quantity</label><input name="quantity" type="number" value={form.quantity} onChange={handleChange} min="0" step="0.01" /></div>
