...
export default function StockMovements() {
  ...
  useEffect(() => {
    masterService.getAll("products").then((res) => setProducts(res.data?.data || res.data || [])).catch(() => {});
    ...
    <MainLayout>
      <div className="card">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between mb-3">
          <strong className="ct" style={{ fontSize: 15, flexShrink: 0 }}>Stock Movements</strong>
          ...
          <table className="w-full">
            ...
            <tbody>
              {loading ? (
                ...
                <tr><td colSpan={6} style={{ textAlign: "center", color: "var(--tx3)", padding: "24px 0" }}>No stock movements found</td></tr>
              ) : paged.map((r) => {
                const s = typeStyle(r.movement_type);
                return (
                  <tr key={r.id}>
                    <td><strong style={{ color: "var(--tx)" }}>{r.product?.name || r.product_id}</strong></td>
                    ...
                    <td>
                      <span style={{ color: r.quantity >= 0 ? "var(--green)" : "var(--red)", fontWeight: 600 }}>
                        {r.quantity > 0 ? "+" : ""}{Number(r.quantity).toLocaleString()}
                      </span>
                    </td>
                    <td className="hidden md:table-cell" style={{ color: "var(--tx3)", fontSize: 12 }}>{r.reference_type || "—"}</td>
                    <td className="hidden lg:table-cell" style={{ color: "var(--tx3)" }}>{r.notes || "—"}</td>
        ...
        {!loading && filtered.length > 0 && (
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between mt-4" style={{ fontSize: 12, color: "var(--tx3)" }}>
            <span>Showing <strong style={{ color: "var(--tx)" }}>{(currentPage - 1) * pageSize + 1}–{Math.min(currentPage * pageSize, filtered.length)}</strong> of <strong style={{ color: "var(--tx)" }}>{filtered.length}</strong> movements</span>
            {totalPages > 1 && (
        ...
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.45)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1000 }}>
          <div className="fcard" style={{ width: 420, maxWidth: "calc(100vw - 32px)", padding: 24 }}>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 16 }}>
              ...
              <button className="hbtn" onClick={() => { setShowModal(false); setForm({ ...EMPTY_ADJ }); }}><X size={14} /></button>
            </div>
            ...
            <div style={{ display: "flex", gap: 8, justifyContent: "flex-end", marginTop: 16 }}>
              <Button variant="ghost" size="sm" onClick={() => { setShowModal(false); setForm({ ...EMPTY_ADJ }); }}>Cancel</Button>
