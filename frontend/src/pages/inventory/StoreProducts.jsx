...
export default function StoreProducts() {
  ...
  const handleDelete = async () => {
    try {
      ...
      toast.success("Product removed from store inventory"); load();
    } catch { toast.error("Delete failed"); }
    finally { setDeleteDialog({ open: false, storeId: null, productId: null, label: "" }); }
  };
    ...
    <MainLayout>
      <div className="card">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between mb-3">
          <strong className="ct" style={{ fontSize: 15, flexShrink: 0 }}>Store Inventory</strong>
          ...
          <table className="w-full">
            ...
            <tbody>
              {loading ? (
                ...
                <tr><td colSpan={7} style={{ textAlign: "center", color: "var(--tx3)", padding: "24px 0" }}>No inventory records found</td></tr>
              ) : paged.map((r, i) => {
                ...
                return (
                  <tr key={`${r.store_id}-${r.product_id}-${i}`} style={{ cursor: "pointer" }} onClick={() => openEdit(r)}>
                    <td><strong style={{ color: "var(--tx)" }}>{r.product?.name || "—"}</strong></td>
                    <td className="hidden sm:table-cell" style={{ fontFamily: "monospace", fontSize: 11, color: "var(--tx3)" }}>{r.product?.sku || "—"}</td>
                    <td className="hidden md:table-cell" style={{ color: "var(--tx2)", fontSize: 12 }}>{storeMap[r.store_id] || "—"}</td>
                    <td>
                    ...
                    <td className="hidden lg:table-cell" style={{ color: "var(--tx3)", fontSize: 12 }}>{Number(r.low_stock_alert).toLocaleString()}</td>
                    <td className="hidden xl:table-cell" style={{ color: "var(--tx3)", fontSize: 11 }}>{r.updated_at ? new Date(r.updated_at).toLocaleDateString() : "—"}</td>
                    <td onClick={e => e.stopPropagation()} style={{ textAlign: "right" }}>
                      <div style={{ display: "flex", alignItems: "center", justifyContent: "flex-end", gap: 4 }}>
                        <button className="hbtn" onClick={e => { e.stopPropagation(); openEdit(r); }} title="Edit"><Pencil size={12} /></button>
                        <button className="hbtn" onClick={e => { e.stopPropagation(); setDeleteDialog({ open: true, storeId: r.store_id, productId: r.product_id, label: r.product?.name }); }} title="Remove"><Trash2 size={12} /></button>
                      </div>
                    </td>
                  </tr>
                );
      ...
      {modal && (
        <Modal isOpen onClose={closeModal} title={modal === "edit" ? "Update Store Inventory" : "Add Product to Store"} width={480}
          footer={<><Button variant="ghost" size="sm" onClick={closeModal}>Cancel</Button><Button variant="primary" size="sm" disabled={saving} onClick={handleSave}>{saving ? "Saving..." : modal === "edit" ? "Update" : "Add"}</Button></>}
        >
          <div className="fg">
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
            ...
            <div className="fg"><label>Low Stock Alert</label><input name="low_stock_alert" type="number" value={form.low_stock_alert} onChange={handleChange} min="0" step="0.01" /></div>
          </div>
        </Modal>
      )}
