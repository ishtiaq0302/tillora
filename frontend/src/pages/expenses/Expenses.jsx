...
export default function Expenses() {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      toast.error(t("failed_to_load", "expenses"));
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const openCreate = () => {
    setForm({ ...EMPTY, expense_date: new Date().toISOString().slice(0, 10), store_id: currentStore?.id || "" });
    setErrors({});
    setEditId(null);
    setModal("create");
  };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (modal === "edit") {
        await masterService.update("expenses", editId, payload);
        toast.success(t("expense_updated", "expenses"));
      } else {
        await masterService.create("expenses", payload);
        toast.success(t("expense_created", "expenses"));
      }
      closeModal();
      loadData();
    } catch (err) {
  ...
  const handleDelete = async () => {
    try {
      await masterService.delete("expenses", deleteDialog.id);
      toast.success(t("expense_deleted", "expenses"));
      loadData();
    } catch {
    ...
    <MainLayout>
              ...
              {loading ? (
                ...
                paged.map((r) => (
                  <tr key={r.id} style={{ cursor: "pointer" }} onClick={() => openEdit(r)}>
                    <td>
                      <span style={{ color: "var(--tx)", fontWeight: 500 }}>{r.expense_date}</span>
                    </td>
                    ...
                    <td>
                      <span style={{ color: "var(--red)", fontWeight: 600 }}>{Number(r.amount).toLocaleString()}</span>
                    </td>
                    <td className="hidden lg:table-cell" style={{ color: "var(--tx3)", maxWidth: 200 }}>
                      {r.notes || "—"}
                    </td>
                    <td onClick={(e) => e.stopPropagation()} style={{ textAlign: "right" }}>
                      <div style={{ display: "flex", alignItems: "center", justifyContent: "flex-end", gap: 4 }}>
                        ...
                        <button
                          ...
                          onClick={(e) => {
                            e.stopPropagation();
                            setDeleteDialog({ open: true, id: r.id, label: `expense on ${r.expense_date}` });
                          }}
                        >
                          <Trash2 size={12} />
                        </button>
        ...
        {!loading && filtered.length > 0 && (
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between mt-4" style={{ fontSize: 12, color: "var(--tx3)" }}>
            <span>
              ...
              {t("of", "common")} <strong style={{ color: "var(--tx)" }}>{filtered.length}</strong> {t("title", "expenses").toLowerCase()}
            </span>
            {totalPages > 1 && (
      ...
      {modal && (
        <Modal
          isOpen
          onClose={closeModal}
          title={modal === "edit" ? t("edit_expense", "expenses") : t("add_expense", "expenses")}
          columns={{ sm: 1, md: 2 }}
          width={520}
          footer={
            <>
        ...
        >
          ...
          <div className="fg">
            <label>
              {t("amount", "common")} <span style={{ color: "var(--red)" }}>*</span>
            </label>
            <input name="amount" type="number" step="0.01" min="0" value={form.amount} onChange={handleChange} style={errors.amount ? { borderColor: "var(--red)" } : {}} placeholder="0.00" />
            {errors.amount && <span style={{ fontSize: 11, color: "var(--red)" }}>{errors.amount}</span>}
          </div>
          <div className="fg">
            <label>
              {t("date", "common")} <span style={{ color: "var(--red)" }}>*</span>
            </label>
            <input name="expense_date" type="date" value={form.expense_date} onChange={handleChange} style={errors.expense_date ? { borderColor: "var(--red)" } : {}} />
          ...
          {currentStore?.id == null && stores.length > 0 && (
            <div className="fg" style={{ gridColumn: "1 / -1" }}>
              <label>
                {t("store", "common")} <span style={{ color: "var(--red)" }}>*</span>
              </label>
