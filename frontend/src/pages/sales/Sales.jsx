...
export default function Sales() {
  ...
  const loadData = useCallback(async () => {
    setLoading(true);
    try {
      const res = await masterService.getAll("sales");
      setData(normalizeListResponse(res?.data ?? res));
    } catch {
      toast.error(t("failed_to_load", "sales"));
    } finally {
      setLoading(false);
    }
  }, [t]);
  ...
  const filtered = useMemo(() => {
    return data.filter((r) => {
      ...
      const matchSearch = !q || (r.invoice_no || "").toLowerCase().includes(q) || (r.customer?.name || "").toLowerCase().includes(q);
  ...
  const handleDelete = async () => {
    try {
      ...
      await Promise.all(ids.map((id) => masterService.delete("sales", id)));
    ...
    <MainLayout>
      ...
      {viewSale && (
        <Modal
          ...
          title={`${t("sale_detail_title", "sales")} — ${viewSale.invoice_no}`}
          columns={{ sm: 1, md: 2 }}
          width={640}
          footer={
            <div style={{ display: "flex", gap: 8 }}>
              <Button variant="ghost" size="sm" onClick={() => printSaleInvoice(viewSale)} style={{ gap: 5 }}>
                <Printer size={13} /> Print
              </Button>
              <Button variant="ghost" size="sm" onClick={() => setViewSale(null)}>
                {t("close", "common")}
              </Button>
            </div>
          }
        >
          <div className="fg">
            <label>{t("invoice", "common")}</label>
            <p style={{ fontSize: 13, color: "var(--tx)", fontWeight: 600 }}>{viewSale.invoice_no}</p>
          </div>
          <div className="fg">
            <label>{t("customer", "common")}</label>
            <p style={{ fontSize: 13, color: "var(--tx)" }}>{viewSale.customer?.name || t("walk_in", "common")}</p>
          </div>
          <div className="fg">
            <label>{t("subtotal", "common")}</label>
            <p style={{ fontSize: 13, color: "var(--tx)" }}>{Number(viewSale.subtotal).toLocaleString()}</p>
          </div>
          <div className="fg">
            <label>{t("discount", "common")}</label>
            <p style={{ fontSize: 13, color: "var(--tx)" }}>{Number(viewSale.discount).toLocaleString()}</p>
          </div>
          <div className="fg">
            <label>{t("tax", "common")}</label>
            <p style={{ fontSize: 13, color: "var(--tx)" }}>{Number(viewSale.tax).toLocaleString()}</p>
          </div>
          <div className="fg">
            <label>{t("grand_total", "common")}</label>
            <p style={{ fontSize: 15, color: "var(--green)", fontWeight: 700 }}>{Number(viewSale.grand_total).toLocaleString()}</p>
          </div>
          <div className="fg">
            <label>{t("payment_status", "common")}</label>
            <span style={{ ...statusStyle(viewSale.payment_status), borderRadius: "var(--r)", padding: "3px 10px", fontSize: 12, fontWeight: 600, textTransform: "capitalize", display: "inline-block" }}>{viewSale.payment_status}</span>
          </div>
          <div className="fg">
            <label>{t("sale_status", "sales")}</label>
            <p style={{ fontSize: 13, color: "var(--tx)", textTransform: "capitalize" }}>{viewSale.sale_status}</p>
          </div>
          <div className="fg">
            <label>{t("date", "common")}</label>
            <p style={{ fontSize: 13, color: "var(--tx)" }}>{viewSale.created_at ? new Date(viewSale.created_at).toLocaleString() : "—"}</p>
          </div>
          {viewSale.notes && (
            <div className="fg" style={{ gridColumn: "1 / -1" }}>
              <label>{t("notes", "common")}</label>
              <p style={{ fontSize: 13, color: "var(--tx)" }}>{viewSale.notes}</p>
            </div>
          )}
                    ...
                    <tbody>
                      {viewSale.saleItems.map((item, i) => (
                        <tr key={i}>
                          ...
                          {hasVariants && <td style={{ fontSize: 11 }}>{item.variant_label ? <span style={{ color: "var(--accent)", fontWeight: 500 }}>{item.variant_label}</span> : <span style={{ color: "var(--tx3)" }}>—</span>}</td>}
                          <td>{item.quantity}</td>
                          <td>
                            {item.variant_price > 0 ? (
                              <>
                                <div style={{ color: "var(--tx3)", fontSize: 11 }}>{Number(item.price - item.variant_price).toLocaleString()}</div>
      ...
      <div className="card">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between mb-3">
          ...
          <div className="srch w-full sm:w-auto sm:flex-1 sm:max-w-xs">
            ...
            <input type="text" placeholder={t("search_placeholder", "sales")} value={search} onChange={(e) => setSearch(e.target.value)} />
          </div>
          <div className="flex items-center justify-between sm:justify-end gap-2 flex-wrap">
            {selectedItems.length > 0 && (
              <Button variant="danger" size="sm" onClick={() => setDeleteDialog({ open: true, id: selectedItems, label: `${selectedItems.length} sales` })}>
            ...
            <Link to="/sales/create" className="btn btn-p" style={{ fontSize: 12 }}>
              <Plus size={13} strokeWidth={2.5} />
              <span>{t("new_sale", "sales")}</span>
            </Link>
          </div>
        </div>
          ...
          <table className="w-full">
            <thead>
              <tr>
                ...
                <th className="hidden sm:table-cell">{t("customer", "common")}</th>
                  ...
                  return (
                    <tr key={r.id}>
                      <td onClick={(e) => e.stopPropagation()}>
                        <input type="checkbox" checked={selectedItems.includes(r.id)} onChange={() => toggleSelect(r.id)} style={{ width: 12, height: 12, accentColor: "var(--accent)", cursor: "pointer" }} />
                      </td>
                      <td>
                        <strong style={{ color: "var(--tx)" }}>{r.invoice_no}</strong>
                      </td>
                      <td className="hidden sm:table-cell">{r.customer?.name || t("walk_in", "common")}</td>
                      <td className="hidden md:table-cell" style={{ color: "var(--tx3)" }}>
                        {r.store?.name || "—"}
                      </td>
                      <td>
                        <span style={{ color: "var(--green)", fontWeight: 600 }}>{Number(r.grand_total).toLocaleString()}</span>
                      </td>
                      <td>
                        <span style={{ background: s.bg, color: s.color, borderRadius: "var(--r)", padding: "2px 8px", fontSize: 11, fontWeight: 600, textTransform: "capitalize" }}>{t(r.payment_status, "common")}</span>
                      </td>
                      ...
                      <td>
                        <div style={{ display: "flex", alignItems: "center", gap: 4 }}>
                          <button className="hbtn" onClick={() => setViewSale(r)} title="View">
                            <Eye size={13} />
                          </button>
                          <button className="hbtn" onClick={() => setDeleteDialog({ open: true, id: r.id, label: r.invoice_no })} title="Delete" style={{ color: "var(--red)" }}>
                            <Trash2 size={12} />
                          </button>
                        </div>
                      </td>
        ...
        {!loading && filtered.length > 0 && (
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between mt-4" style={{ fontSize: 12, color: "var(--tx3)" }}>
            <span>
              ...
              {t("of", "common")} <strong style={{ color: "var(--tx)" }}>{filtered.length}</strong> {t("title", "sales").toLowerCase()}
            </span>
            {totalPages > 1 && (
