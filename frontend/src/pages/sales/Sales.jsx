...
export default function Sales() {
  ...
  const loadData = useCallback(async () => {
    ...
    } catch { toast.error(t("failed_to_load", "sales")); }
  ...
  return (
    <MainLayout>
      ...
      {/* DETAIL MODAL */}
      {viewSale && (
        <Modal isOpen onClose={() => setViewSale(null)} title={`${t("sale_detail_title", "sales")} — ${viewSale.invoice_no}`}
          ...
          footer={
            <div style={{ display: "flex", gap: 8 }}>
              ...
              <Button variant="ghost" size="sm" onClick={() => setViewSale(null)}>{t("close", "common")}</Button>
            </div>
          }>
          <div className="fg"><label>{t("invoice", "common")}</label><p style={{ fontSize: 13, color: "var(--tx)", fontWeight: 600 }}>{viewSale.invoice_no}</p></div>
          <div className="fg"><label>{t("customer", "common")}</label><p style={{ fontSize: 13, color: "var(--tx)" }}>{viewSale.customer?.name || t("walk_in", "common")}</p></div>
          <div className="fg"><label>{t("subtotal", "common")}</label><p style={{ fontSize: 13, color: "var(--tx)" }}>{Number(viewSale.subtotal).toLocaleString()}</p></div>
          <div className="fg"><label>{t("discount", "common")}</label><p style={{ fontSize: 13, color: "var(--tx)" }}>{Number(viewSale.discount).toLocaleString()}</p></div>
          <div className="fg"><label>{t("tax", "common")}</label><p style={{ fontSize: 13, color: "var(--tx)" }}>{Number(viewSale.tax).toLocaleString()}</p></div>
          <div className="fg"><label>{t("grand_total", "common")}</label><p style={{ fontSize: 15, color: "var(--green)", fontWeight: 700 }}>{Number(viewSale.grand_total).toLocaleString()}</p></div>
          <div className="fg">
            <label>{t("payment_status", "common")}</label>
            <span style={{ ...statusStyle(viewSale.payment_status), borderRadius: "var(--r)", padding: "3px 10px", fontSize: 12, fontWeight: 600, textTransform: "capitalize", display: "inline-block" }}>
              {viewSale.payment_status}
            </span>
          </div>
          <div className="fg"><label>{t("sale_status", "sales")}</label><p style={{ fontSize: 13, color: "var(--tx)", textTransform: "capitalize" }}>{viewSale.sale_status}</p></div>
          <div className="fg"><label>{t("date", "common")}</label><p style={{ fontSize: 13, color: "var(--tx)" }}>{viewSale.created_at ? new Date(viewSale.created_at).toLocaleString() : "—"}</p></div>
          {viewSale.notes && (
            <div className="fg" style={{ gridColumn: "1 / -1" }}><label>{t("notes", "common")}</label><p style={{ fontSize: 13, color: "var(--tx)" }}>{viewSale.notes}</p></div>
          )}
      ...
      <div className="card">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between mb-3">
          <strong className="ct" style={{ fontSize: 15, flexShrink: 0 }}>{t("title", "sales")}</strong>
          ...
          <div className="flex items-center justify-between sm:justify-end gap-2 flex-wrap">
            ...
            <Link to="/sales/create" className="btn btn-p" style={{ fontSize: 12 }}>
              <Plus size={13} strokeWidth={2.5} /><span>{t("new_sale", "sales")}</span>
            </Link>
          </div>
        </div>
          ...
          <table className="w-full">
            ...
            <tbody>
              {loading ? (
                ...
                <tr><td colSpan={8} style={{ textAlign: "center", color: "var(--tx3)", padding: "24px 0" }}>{t("no_sales_found", "sales")}</td></tr>
              ) : paged.map((r) => {
                  ...
                  <tr key={r.id}>
                    ...
                    <td>
                      <div style={{ display: "flex", alignItems: "center", gap: 4 }}>
                        <button className="hbtn" onClick={() => setViewSale(r)} title="View"><Eye size={13} /></button>
        ...
        {!loading && filtered.length > 0 && (
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between mt-4" style={{ fontSize: 12, color: "var(--tx3)" }}>
            <span>{t("showing", "common")} <strong style={{ color: "var(--tx)" }}>{(currentPage - 1) * pageSize + 1}–{Math.min(currentPage * pageSize, filtered.length)}</strong> {t("of", "common")} <strong style={{ color: "var(--tx)" }}>{filtered.length}</strong> {t("title", "sales").toLowerCase()}</span>
            {totalPages > 1 && (
