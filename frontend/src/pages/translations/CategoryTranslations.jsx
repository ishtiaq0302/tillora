...
const EMPTY = { category_id: "", language_id: "", name: "" };
...
export default function CategoryTranslations() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const res = await masterService.getAll("category-translations");
      setData(res.data?.data || res.data || []);
    } catch { toast.error("Failed to load category translations"); }
  ...
  const filtered = useMemo(() => data.filter(r => {
    const matchCat = !filterCat || r.category_id === filterCat;
    const matchLang = !filterLang || r.language_id === filterLang;
    const matchSearch = !search.trim() || (r.name || "").toLowerCase().includes(search.toLowerCase()) || (r.category?.name || "").toLowerCase().includes(search.toLowerCase());
    return matchCat && matchLang && matchSearch;
  }), [data, search, filterCat, filterLang]);

  const totalPages = Math.ceil(filtered.length / pageSize);
  const paged = useMemo(() => filtered.slice((currentPage - 1) * pageSize, currentPage * pageSize), [filtered, currentPage, pageSize]);
  useEffect(() => setCurrentPage(1), [search, filterCat, filterLang, pageSize]);
  ...
  const openEdit = (r) => {
    setForm({ category_id: r.category_id, language_id: r.language_id, name: r.name });
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      await masterService.create("category-translations", { category_id: form.category_id, language_id: form.language_id, name: form.name.trim() });
  ...
  const handleDelete = async () => {
    try { await masterService.delete("category-translations", deleteDialog.id); toast.success("Translation deleted"); load(); }
    ...
    <MainLayout>
      <div className="card">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between mb-3">
          <strong className="ct" style={{ fontSize: 15, flexShrink: 0 }}>Category Translations</strong>
          <div className="srch w-full sm:w-auto sm:flex-1 sm:max-w-xs">
            <span className="srch-ic"><Search size={13} /></span>
            <input type="text" placeholder="Search category or translated name..." value={search} onChange={e => setSearch(e.target.value)} />
          </div>
          <div className="flex items-center gap-2 flex-wrap">
            <select value={filterCat} onChange={e => setFilterCat(e.target.value)} style={psStyle}>
          ...
          <table className="w-full">
            ...
            <tbody>
              {loading ? (
                ...
                <tr><td colSpan={4} style={{ textAlign: "center", color: "var(--tx3)", padding: "24px 0" }}>No category translations found</td></tr>
              ) : paged.map(r => (
                <tr key={r.id} style={{ cursor: "pointer" }} onClick={() => openEdit(r)}>
                  <td style={{ color: "var(--tx2)", fontSize: 12 }}>{r.category?.name || "—"}</td>
                  <td>
                  ...
                  <td onClick={e => e.stopPropagation()} style={{ textAlign: "right" }}>
                    <div style={{ display: "flex", alignItems: "center", justifyContent: "flex-end", gap: 4 }}>
                      ...
                      <button className="hbtn" onClick={e => { e.stopPropagation(); setDeleteDialog({ open: true, id: r.id, label: `${r.category?.name} (${r.language?.name})` }); }} title="Delete"><Trash2 size={12} /></button>
                    </div>
                  </td>
                </tr>
              ))}
      ...
      {modal && (
        <Modal isOpen onClose={closeModal} title={modal === "edit" ? "Edit Category Translation" : "Add Category Translation"} width={460}
        ...
        >
          <div className="fg">
            ...
            <select name="category_id" value={form.category_id} onChange={handleChange} style={errors.category_id ? { borderColor: "var(--red)" } : {}}>
              <option value="">Select category...</option>
              {categories.map(c => <option key={c.id} value={c.id}>{c.name}</option>)}
            </select>
            {errors.category_id && <span style={{ fontSize: 11, color: "var(--red)" }}>{errors.category_id}</span>}
          </div>
          <div className="fg">
          ...
          <div className="fg">
            ...
            <input name="name" value={form.name} onChange={handleChange} placeholder="Category name in selected language" autoFocus style={errors.name ? { borderColor: "var(--red)" } : {}} />
