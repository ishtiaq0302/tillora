...
export default function StoreLanguages() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      toast.error("Failed to load store languages");
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const handleDelete = async () => {
    try {
      await masterService.delete("store-languages", deleteDialog.id);
      toast.success("Language removed from store");
      load();
    } catch {
    ...
    <MainLayout>
              ...
              {loading ? (
                ...
                paged.map((r) => (
                  <tr key={r.id} style={{ cursor: "pointer" }} onClick={() => openEdit(r)}>
                    ...
                    <td>
                      <strong style={{ color: "var(--tx)" }}>{r.language?.name || "—"}</strong>
                    </td>
                    <td>
                    ...
                    <td>
                      {r.is_default ? (
                        <span className="sta ok" style={{ fontSize: 11 }}>
                          Default
                        </span>
                      ) : (
                    ...
                    <td onClick={(e) => e.stopPropagation()} style={{ textAlign: "right" }}>
                      <div style={{ display: "flex", alignItems: "center", justifyContent: "flex-end", gap: 4 }}>
                        ...
                        <button
                          ...
                          onClick={(e) => {
                            e.stopPropagation();
                            setDeleteDialog({ open: true, id: r.id, label: `${storeMap[r.store_id]} – ${r.language?.name}` });
                          }}
                          title="Remove"
                        >
                          <Trash2 size={12} />
      ...
      {modal && (
        <Modal
          isOpen
          onClose={closeModal}
          title={modal === "edit" ? "Edit Store Language" : "Assign Language to Store"}
          width={460}
          footer={
            <>
              ...
              <Button variant="primary" size="sm" disabled={saving} onClick={handleSave}>
                {saving ? "Saving..." : modal === "edit" ? "Update" : "Assign"}
              </Button>
            </>
          }
        >
          ...
          <div className="fg" style={{ flexDirection: "row", alignItems: "center", gap: 10 }}>
            ...
            <label htmlFor="is_default" style={{ cursor: "pointer", marginBottom: 0 }}>
              Set as default language for this store
            </label>
          </div>
        </Modal>
      )}
