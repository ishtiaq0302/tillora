    ...
    <>
      <div className="fg">
        ...
        <input name="name" value={form.name} onChange={onChange} autoFocus
          style={errors.name ? { borderColor: "var(--red)" } : {}} placeholder="e.g. Nestle" />
...
export default function Brands() {
  ...
  return (
    <CrudPage
      title={t("brands", "nav")}
      singular={t("singular_brand", "master")}
      resource="brands"
      ...
      emptyForm={{ name: "", store_id: currentStore?.id || "", is_active: true }}
      toForm={(r) => ({ name: r.name, store_id: r.store_id || "", is_active: r.is_active ?? true })}
      toPayload={(f) => ({ name: f.name.trim(), store_id: f.store_id || null, is_active: f.is_active })}
      validate={(f) => {
        const e = {};
        if (!f.name.trim()) e.name = t("brand_name_required", "master");
        return e;
      }}
